	...
	return (
		<div className="flex flex-col h-screen bg-[#020617] text-slate-100">
			...
			<div className="p-4 border-b border-white/5 flex items-center gap-4 bg-slate-900/40 backdrop-blur-md sticky top-0 z-20">
				<button
					className="w-10 h-10 rounded-2xl bg-slate-950/40 border border-white/5 flex items-center justify-center active:scale-95 transition-all"
					onClick={() => router.back()}
				>
					<ArrowLeft className="w-5 h-5 text-slate-500" />
				</button>
				<div>
					<h1 className="text-base font-black italic flex items-center gap-2">
						<Sparkles className="w-4 h-4 text-emerald-400" /> AI Asisten
					</h1>
			...
			<div className="flex-1 overflow-y-auto p-4 space-y-6 pb-48">
				{messages.map((msg, i) => (
					...
					>
						...
						<div
							className={cn(
								"p-3 rounded-[1.2rem] text-xs font-bold max-w-[85%] shadow-premium italic leading-relaxed",
								msg.role === "user"
			...
			<div className="fixed bottom-20 left-1/2 -translate-x-1/2 w-full max-w-md p-4 bg-slate-950/40 backdrop-blur-xl border-t border-white/5 space-y-4 z-20">
				...
				<div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
					{CHAT_SUGGESTIONS.map((s) => (
						<button
							key={s}
							onClick={() => setInput(s)}
							className="whitespace-nowrap px-4 py-2 bg-slate-900 border border-white/5 rounded-full text-[10px] font-bold text-slate-400 hover:text-emerald-400 hover:border-emerald-500/30 transition-all active:scale-95 shrink-0 shadow-sm"
						>
							{s}
						</button>
					))}
				...
				<div className="flex gap-3">
					<Input
						...
						className="flex-1 bg-slate-900/60 border-white/5 h-12 rounded-2xl font-bold italic shadow-inner px-5 text-xs text-slate-100 placeholder:text-slate-700 focus-visible:ring-emerald-500/30"
						value={input}
					...
					<Button
						size="icon"
						className="h-12 w-12 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white shadow-xl shadow-emerald-950/30 border-b-4 border-emerald-800 active:border-b-0 active:translate-y-1 transition-all"
						onClick={() => sendMessage()}
					>
